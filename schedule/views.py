...
def schedule_list_view(request):
    ...
    schedules = Sleep_schedule.objects.all().order_by('date') #list of object
    # schedules = Sleep_schedule.objects.filter(date__gte = current_date).order_by('date') #list of object
...
def dynamic_schedule_lookup_view(request, id):
    # obj = Product.objects.get(id=id)
    sleep_schedule = get_object_or_404(Sleep_schedule, id = id)
